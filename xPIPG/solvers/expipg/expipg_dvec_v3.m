function [x1,u1,w1,exit_flag] = expipg_dvec_v3(x1,u1,w1,...                                          % Main solution variables
...
%}
    ...
    for k = 1:k_max % Run algorithm for at most k_max iterations 
        ...
        if rem(k,k_test) == 0 % Test stopping criteria only every k_test iterations
            ...
            err_p1 = max(err_p1, max((x_N - x1(:,N+1)) .* max(x2(:,N+1),0)));
            err_p1 = max(err_p1, max((x_N - x1(:,N+1)) .* min(x2(:,N+1),0)));            
