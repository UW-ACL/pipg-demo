...
M_avg = 10;     % Number of solves to average over for each tolerance value
...
for j = 1:M_eps
    ...
    for k = 1:M_avg
        ...
        if sol_expipg.status == "Feasible"
            ...
            y = - ppv.P*sol_expipg.xi - ppv.HT*sol_expipg.eta;
            constr_viol.expipg(j) = constr_viol.expipg(j) + max([constraint_violation(sol_expipg,ppv), max((ppv.xi_min - sol_expipg.xi) .* min(y,0)), max((ppv.xi_max - sol_expipg.xi) .* max(y,0))]);
            % constr_viol.expipg(j) = constr_viol.expipg(j) + max([constraint_violation(sol_expipg,ppv), (ppv.xi_min - sol_expipg.xi)' * min(y,0), (ppv.xi_max - sol_expipg.xi)' * max(y,0)]);
