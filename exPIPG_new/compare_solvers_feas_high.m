...
for j = 1:M_avg
    ...
    % OSQP    
    pp.osqp_abs_tol = cv_struct.osqp_eps_high;
    pp.osqp_rel_tol = eps;
    ...
    % SCS
    pp.scs_abs_tol = cv_struct.scs_eps_high;
    pp.scs_rel_tol = eps;
    ...
    % MOSEK
    pp.mosek_feas_tol = cv_struct.mosek_eps_high;
    pp.mosek_rel_gap = cv_struct.mosek_eps_high;
    pp.mosek_infeas_tol = eps;
    sol_mosek = solve_mosek(pp,ppv);
    constr_viol.mosek(j) = constraint_violation(sol_mosek,ppv);

    % exPIPG
    pp.expipg_feas_tol = cv_struct.expipg_eps_high;
    pp.expipg_infeas_tol = eps;
    sol_expipg = solve_expipg(pp,ppv,{'DVEC','mex','new_infeas'});

    % ECOS    
    pp.ecos_abs_tol = cv_struct.ecos_eps_high;
    pp.ecos_rel_tol = eps;
    pp.ecos_feas_tol = cv_struct.ecos_eps_high;
...
save(horzcat('results/feas_high_time_n',num2str(n),'_m',num2str(m),'_N',num2str(N)),'solve_time','M_pipg','M_avg','log10_eps_low','constr_viol');
